...
CREATE TABLE risk_profiles (
    ...
    martingale_steps            INTEGER NOT NULL DEFAULT 1
                                CHECK (martingale_steps >= 1 AND martingale_steps <= 1),
...
CREATE TABLE trades (
    ...
    result                  VARCHAR(20) CHECK (result IN ('win', 'loss', 'tie', 'cancelled')),
    user_notes              TEXT,
    created_at              TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE TABLE martingale_states (
    ...
    user_id             UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    instrument          VARCHAR(30) NOT NULL,
    current_step        VARCHAR(10) NOT NULL DEFAULT '0',
